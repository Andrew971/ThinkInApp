...
const apiUrl = 'http://localhost:8080/lab/'
...
export function* GetList(action) {
	...
	const res = yield call(axios.post, apiUrl + 'getlist', { forumId: action.info })
...
export function* updateLab(action) {
	const res = yield call(axios.post, apiUrl + 'update/' + action.info.labId, action.info)
	yield put({ type: 'LAB_UPDATE_SUCCESS', payload: res.data })

}

export function* AddComment(action) {
	const res =  yield call(axios.post, apiUrl+'comment/'+action.info.labId,{comment: action.info.comment, owner: action.info.owner})
 yield put({ type: 'ADD_COMMENT_SUCCESS', payload: res.data })
}
export function* GetComment(action) {
	const res = yield call(axios.post, apiUrl+'comment/get/'+action.info.labId)
...
export function* deleteLab(action) {
	...
	const res = yield call(axios.post, apiUrl + action.info.labId)
